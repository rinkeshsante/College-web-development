...
from .models import Lab, Equipment, Software, ComputerSoftwareMapping
from django.contrib.auth.decorators import login_required, user_passes_test
from .forms import LabForm, EquipmentForm, SoftwareForm, ComputerSoftwareMappingForm
...
@login_required
def ComputerSoftwareMappingCreateView(request):
    form = ComputerSoftwareMappingForm()
    if request.method == 'POST':
        form = ComputerSoftwareMappingForm(request.POST)
        if form.is_valid():
            form.save()
...
@login_required
def ComputerSoftwareMappingUpdateView(request, num):
    obj = ComputerSoftwareMapping.objects.get(id=num)
    form = ComputerSoftwareMappingForm(instance=obj)
    if request.method == 'POST':
        form = ComputerSoftwareMappingForm(request.POST, instance=obj)
        if form.is_valid():
            form.save()
            return redirect('dashboard', num)
...
@login_required
def ComputerSoftwareMappingDeleteView(request, num):
    obj = ComputerSoftwareMapping.objects.get(id=num)
    if request.method == 'POST':
        obj.delete()
        return redirect('dashboard')
